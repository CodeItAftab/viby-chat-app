...
export function ChatView({
  ...
  const { data: RealMessages } = useGetMessagesQuery(chatId || "");
  console.log("Messages:", RealMessages);
  ...
  const handleSendMessage = (
    content: string,
    type: "text" | "image" | "video" | "audio" | "file" | "raw",
    files?: File[]
  ) => {
    // onSendMessage(content, type, files);
    ...
    // }
    console.log(content, type, files);
  };
  ...
  //     // Ensure createdAt exists, fallback to current date if not
  ...
  // ) => {
  //   // Add null checks for both messages
  ...
  //   const twoMinutes = 2 * 60 * 1000; // Reduced from 5 minutes to 2 minutes for tighter grouping

  //   return (
  //     currentMessage.sender === previousMessage.sender &&
  //     currentMessage.isOwn === previousMessage.isOwn &&
  //     timeDiff < twoMinutes
  //   );
  // };

  // const messageGroups = groupMessagesByDate(RealMessages || []);
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-white dark:bg-slate-900">
      ...
      {/* Enhanced Messages Area with Date Separators */}
      <div className="flex-1 overflow-y-auto bg-gradient-to-b from-white via-slate-50 to-blue-50 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950">
        <div className="p-3 sm:p-6">
          ...
          {/* <div className="flex items-center justify-center mb-4">
            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="space-y-0">
            {/* {dateMessages.map((message, index) => {
                  ...
                  //   previousMessage
                  // );
                  // const isLastInGroup = !shouldGroupMessage(
                  //   nextMessage,
                  //   message
                  // );
